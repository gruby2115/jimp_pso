...
// Domyœlne wagi algorytmu (jeœli brak pliku config)
// w = 0.5, c1 = 1.0, c2 = 1.0
#define DEF_W 0.5
#define DEF_C1 1.0
#define DEF_C2 1.0

// Pomocnicza funkcja do wczytania konfiguracji z pliku
int load_config(const char* filename, Parametry* param) {
    FILE* f = fopen(filename, "r");
	if (!f) return 0; // jeœli nie ma pliku, to wczytujemy domyœlne

    // Zak³adamy format pliku: trzy liczby oddzielone spacj¹
...
int main(int argc, char* argv[]) {
    ...
    for (i = 2; i < argc; i++) {
        if (strcmp(argv[i], "-p") == 0 && i + 1 < argc) {
            ...
            if (!load_config(conf_file, &param)) {
                fprintf(stderr, "Nie udalo sie wczytac pliku config: %s. Wczytywanie domyslnych.\n", conf_file);
            }
            else {
	...
	// Przygotowanie pliku logów, jeœli u¿ytkownik tego chce
    if (log_interval > 0) {
